  ...
  if(what %in% dothis){
    ...
    for (i in 1:ll[1]) {
      ...
      for (r in 1:nrrep) {
        ...
        res[r, i] = sagmbAssess(ny@data, sim)     
        ##
        ## if(res[r,i]>0.045) {
...
library("vsn")
options(error=recover)
dat = sagmbSimulateData(n=10000, d=6, de=0, nrstrata=1, miss=0.01, log2scale=TRUE)
v = vsn2(dat$y, lts.quantile=1)
par(mfrow=c(2, 3))
for(j in seq_len(ncol(v))) {
  vj = vsn2(dat$y[,j], reference=v, lts.quantile=1)
  ##plot(dat$hy[,j], exprs(v)[,j])
  plot(dat$hy[,j], exprs(vj))
  ##plot(exprs(v)[,j], exprs(vj))
