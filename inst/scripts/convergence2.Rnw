...
dothis = c("n: one stratum", "n: 8 strata", 
           "d", "nrstrata",
...
%
<<fign1, fig=TRUE, include=FALSE, prefix=FALSE, eps=FALSE>>=
## with one stratum
n   = 1000*2^seq(-2, 6)
...
de  = (0:6)/10
res1 = sim(de=de, nrstrata=2, what="de, lts.quantile=0.9")
res2 = sim(de=de, nrstrata=2, lts.quantile=0.5, what="de, lts.quantile=0.5")
...
%
<<figup, fig=TRUE, include=FALSE, prefix=FALSE, width=11, height=5.5>>=
up  = (0:8)/8
res1 = sim(up=up, nrstrata=2, de=0.2, what="up, lts.quantile=0.9")
res2 = sim(up=up, nrstrata=2, de=0.2, lts.quantile=0.5, what="up, lts.quantile=0.5")
...
<<figmiss1, fig=TRUE, include=FALSE, prefix=FALSE, width=5, height=5>>=
miss = seq(0, 0.75, length=7)
res = sim(miss=miss, d=8, what="fraction NA (d=8)")
myPlot(miss, res, log="y")
@ 
%
<<figmiss2, fig=TRUE, include=FALSE, prefix=FALSE, width=5, height=5>>=
...
\begin{figure}[tbp]
\begin{center}
\includegraphics[width=0.49\textwidth]{figmiss1}
\includegraphics[width=0.49\textwidth]{figmiss2}
...
<<figincr, fig=TRUE, include=FALSE, prefix=FALSE, width=9, height=9>>=
par(mfcol=c(2,2))
for(k in 1:2) {
 plot(dat$par[1,,k], parRef[1,,k], xlab="True", ylab="Reference fit", main=c("offset", "factor")[k])
 abline(a=0, b=1)
 plot(fit@par[1,,k], parRef[1,,k], xlab="Profile Likelihood fit", ylab="Reference fit", main=c("offset", "factor")[k])
 abline(a=0, b=1)
}
@ 
...
%------------------------------------------------------
\textbf{TODO}: Some sort of test that checks for correct functioning 
