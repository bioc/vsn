...
sim <- function(..., lts.quantile=0.9, nrrep=20) {
  ...
  ## default parameters
  simpar <- append(callpar, list(n=4096, d=2, de=0, up=0.5, nrstrata=1, log2scale=TRUE)) 
  simpar <- simpar[!duplicated(names(simpar))]
  for (i in 1:ll[1]) {
    simpar[[1]] <- callpar[[1]][i]
    for (r in 1:nrrep) {
      sim <- do.call("sagmbSimulateData", simpar)
      ny  <- vsnMatrix(sim$y, strata=factor(sim$strata), 
...
myPlot <- function(n, res, ...) {
  matplot(n, t(res), pch=20, log="xy", ylab='r.m.s. error', , col="#909090")
...
\caption{\label{figds}%
Estimation error as a function of the number of samples (a) 
and number of strata (b). See Sections~\ref{sec:d} and \ref{sec:nrstrata}.}
\end{center}
\end{figure}
...
up  <- (0:8)/8
res1 <- sim(up=up, de=0.2)
res2 <- sim(up=up, de=0.2, lts.quantile=0.5)
myPlot2(up, list("lts.quantile=0.9"=res1, "lts.quantile=0.5"=res2))
@
%
\begin{figure}[tbp]
\begin{center}
