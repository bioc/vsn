...
sim = function(..., lts.quantile=0.9, nrrep=30L, verbose=FALSE) {
  callpar = list(...)
  ll      = listLen(callpar)
  ...
  ## default parameters
  simpar = append(callpar, list(n=4096L, d=2L, de=0, up=0.5, nrstrata=1L, miss=0, log2scale=TRUE)) 
  simpar = simpar[!duplicated(names(simpar))]
  for (i in 1:ll[1]) {
    if(verbose) cat("i=", i, ": ", sep="")
    ...
    for (r in 1:nrrep) {
      ...
      ny  = vsn2(sim$y, strata=factor(sim$strata), lts.quantile=lts.quantile, verbose=!TRUE,
        optimpar = list(factr=4e4, pgtol=2e-4, lower=1e-6))

      res[r, i] = sagmbAssess(ny@hx, sim)  
      if(verbose) cat(signif(res[r,i], 3), "") 
...
for previous versions of \Rpackage{vsn}: a large region  
