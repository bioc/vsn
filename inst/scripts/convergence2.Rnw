...
myPlot = function(n, res, ...) {
  matplot(n, t(res), pch=20, log="xy", ylab='r.m.s. error', main=attr(res, "main"), col="#909090")
...
<<figmiss>>=
miss = seq(0, 0.8, length=7)
res = sim(miss=miss, what="fraction NA")
myPlot(miss, res)
@ 
\begin{figure}[tbp]
\begin{center}
\includegraphics[width=0.5\textwidth]{figmiss}
% \includegraphics[width=0.5\textwidth]{figincr}
\caption{\label{figds}%
