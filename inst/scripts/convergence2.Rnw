...
sim <- function(..., nrrep=16) {
  ...
  for (i in 1:ll[1]) {
    ...
    for (r in 1:nrrep) {
      ...
      ny  <- vsnMatrix(sim$y, strata=factor(sim$strata), verbose=!TRUE)
...
\caption{\label{figds}%
Estimation error as a function of the number of sample (a) 
...
par(mfrow=c(1,2))
myPlot(de, res1, main="lts.quantile=0.9")
myPlot(de, res2, main="lts.quantile=0.5")
@
%
...
par(mfrow=c(1,2))
myPlot(up, res1, main="lts.quantile=0.9")
myPlot(up, res2, main="lts.quantile=0.5")
@
%
\begin{figure}[tbp]
\begin{center}
