...
<<figmiss1, fig=TRUE, include=FALSE, prefix=FALSE>>=
miss = seq(0, 0.75, length=7)
res = sim(miss=miss, d=8, what="fraction NA (d=8)")
myPlot(miss, res, log="y")
@ 
%
<<figmiss2, fig=TRUE, include=FALSE, prefix=FALSE>>=
...
par(mfcol=c(2,2))
for(k in 1:2) {
 plot(dat$par[1,,k], parRef[1,,k], main=paste("True vs ref-fitted", c("offset", "factor")[k]))
 abline(a=0, b=1)
 plot(fit@par[1,,k], parRef[1,,k], main=paste("profile-fitted vs ref-fitted", c("offset", "factor")[k]))
 abline(a=0, b=1)
}
@ 
...
\begin{figure}[tbp]
\begin{center}
\includegraphics[width=.8\textwidth]{figincr}
