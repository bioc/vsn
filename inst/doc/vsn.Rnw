...
reference~\cite{HuberISMB2002}.  More sophistacted analyses can be
...
statistics. I have generally found the results to be the same.  You
are encouraged to try this out with your own data.

\begin{figure}[htbp]
\begin{center}
\includegraphics[width=\textwidth]{vsn-comp}
\caption{\label{fig-comp}Left hand side: $x$-axis -- generalized
log-ratios for slide 1 from \code{vsn}, $y$-axis -- log-ratios for
slide 1 after global median normalization. For most genes, the two are
the same, but in some cases the generalized log-ratio is smaller.  It
is never larger. This demonstrates the {\em ratio shrinkage} by the
variance stabilization. Right hand side: quantile-quantile-plot of the
$t$-statistic for the comparison between the 4 slides with CLL and the
4 with DLCL. The $t$-statistics from \code{vsn} are larger, i.\,e.\ 
it compares more favorable with respect to the
relative size of between- and within-group variability.
}
\end{center}
\end{figure}
...
to run.  In a version of the vignette (soon!), this section will be
branched out into a separate Sweave file, where CPU runtime is not
...
(submitted to {\em Bioinformatics}, 2003).
