...
\code{vsn} is able to correct for a shift in the position and a
different scaling shift of the data. Suppose the example data were not
so well measured, and the red channel had a baseline that was shifted by 50 and
a scale that differed by a factor of $1/2$:
<<nkid-calib2>>=
bkid = kidney
...
b = vsn(lymphoma, verbose=FALSE)
meanSdPlot(b)
\myincfig{vsn-b-sdmean}{0.5\textwidth}
@
%------------------------------------------------------------
