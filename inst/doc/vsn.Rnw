...
plot(exprs(xnorm), main = "a) vsn", pch = ".", col=ifelse(select, "black", "orange"), asp=1)
plot(log2(exprs(kidney)[select, ]), main = "b) raw", pch = ".", asp=1)
@
%
...
\end{minipage}
%}
\caption{\label{tab:lym}}
\end{table}
%
The 16 columns of the \Robject{lymphoma} object contain the red and
green intensities, respectively, from the 8 slides, as shown in Table~\ref{tab:lym}. 
...
plot(exprs(d.rma)[,1], exprs(d.vsn)[,1], xlab="rma", ylab="vsn", main="chip 1: expression values")
...
adf = new("AnnotatedDataFrame", 
  ...
  varMetadata =  varMetadata(lymphoma[, c(wr, wg)]))
phenoData(lymES) = adf 
@ 
%
One further manipulation is now necessary to ensure the internal consistency of 
...
lf = lmFit(lymM, design[,"sampleClassDLCL", drop=FALSE])
lf = eBayes(lf)
@ 
%
\begin{figure*}[t]\begin{center}
...
matplot(t(exprs(lymM)[order(lf$p.value)[1:5],]), lty=1, type="l", pch=16, col=hsv(seq(0,1,length=5), 1, 0.6), ylab="M", xlab="arrays")
@ 
...
<<>>=
lymRGwbg = lymRG
## lymRG$Rb = 100+30*runif(nrow(lymRG))
## Gb=20+30*runif(nrow(rkid))))
@ 
