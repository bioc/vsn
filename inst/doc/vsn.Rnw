...
%\VignetteDepends{Biobase,vsn,multtest,marrayNorm}
...
pData(lymphoma)
@
The 16 colums of the \code{lymphoma} object contain the red 
...
\code{exprSet}. It is also possible to pass matrices, data frames
with numeric columns only, and objects of class \code{marrayRaw}.
Furthermore, \code{vsn} can be used as a normalization methods 
in the function \code{expresso} in the package 
\code{affy} (cf.~Section~\ref{affy}).

%------------------------------------------------------------
\section{Running vsn on cDNA data} 
%------------------------------------------------------------
Let's try out \code{vsn} on this example data.
To save time, we run it on the intensities from one slide only, 
say lc7b048:
%------------------------------------------------------------
<<a-scp,echo=FALSE,results=hide,include=FALSE,fig=TRUE>>=
if(!exists("a")) 
  a = vsn(lymphoma[, 3:4])
plot(exprs(a), pch = ".")
@
<<a-scp-show,eval=FALSE>>=
a = vsn(lymphoma[, 3:4])
plot(exprs(a), pch = ".")
@
\myincfig{vsn-a-scp}{85mm}
%------------------------------------------------------------
The function returns the transformed intensities in an object of class
\code{exprSet}. Its slot \code{exprs} is a matrix of the same size as
the input data.  The plot in Fig.~\ref{vsn-a-scp} shows the complete
...
\end{equation}
%------------------------------------------------------------
<<a-sdmean,include=FALSE,fig=TRUE,width=8,height=4,results=hide>>=
par(mfrow=c(1,2))
meanSdPlot(a, ranks=TRUE)
meanSdPlot(a, ranks=FALSE)
@
\myincfig{vsn-a-sdmean}{\textwidth}
%------------------------------------------------------------
Such a plot is shown in Fig.~\ref{vsn-a-sdmean}.
...
%------------------------------------------------------------
<<a-iter,include=FALSE,fig=TRUE,width=8,height=4,results=hide>>=
par(mfrow=c(1,2))
vsnPlotPar(a, "offsets")
vsnPlotPar(a, "factors")
@
\myincfig{vsn-a-iter}{0.75\textwidth}
%------------------------------------------------------------
The plots in Fig.~\ref{vsn-a-iter} show the values of the estimated
...
\code{lts.quantile} may be decreased, but more likely, it indicates a
...
%------------------------------------------------------------
<<a-histM,include=FALSE,fig=TRUE,results=hide>>=
M <- exprs(a)[,2] - exprs(a)[,1] 
hist(M, breaks=50)
@
\begin{figure}[htbp]\begin{center}
  \includegraphics[width=0.45\textwidth]{vsn-a-histM}
  \includegraphics[width=0.45\textwidth]{vsn-b-sdmean}
  \caption{\label{vsn-histandsdmean}}
\end{center}\end{figure}
%------------------------------------------------------------
The histogram is shown in the left panel of Fig.~\ref{vsn-a}.
...
%------------------------------------------------------------
<<b-sdmean,echo=FALSE,results=hide,include=FALSE,fig=TRUE>>=
if(!exists("b")) 
  b = vsn(lymphoma, verbose=FALSE)
meanSdPlot(b)
@
<<b-sdmean-show,eval=FALSE>>=
b = vsn(lymphoma, verbose=FALSE)
meanSdPlot(b)
@
%------------------------------------------------------------
...
(see the right panel of Fig.~\ref{vsn-histandsdmean}). 
...
8 samples:
%------------------------------------------------------------
<<b-M,include=FALSE,fig=TRUE,width=8,height=4,results=hide>>=
refrs <- (1:8)*2-1
samps <- (1:8)*2
M <- exprs(b)[,samps] - exprs(b)[,refrs] 
...
abline(h=0, col="red")
@
\myincfig{vsn-b-M}{\textwidth}
%------------------------------------------------------------
Fig.~\ref{vsn-b-M} shows the analagon to the $M$-vs-$A$-plots
described in reference~\cite{Dudoit578}. Note that in the left
...
for \code{vsn} is provided that can be used as module within the 
...
@
\myincfig{vsn-affy}{0.75\textwidth}
...
\newblock {\em Statistical Applications in Genetics and Molecular Biology}, Vol. 2: No. 1, Article 3, 2003. 
