...
The function \code{calc.fdr} provides a simple-minded 
...
the probe sets. The choices made in the following example may not be 
optimal.
<<affy,include=FALSE,fig=TRUE>>==
example(normalize.AffyBatch.vsn)
@
\myincfig{vsn-affy}{70mm}
The scatterplot is shown in Fig.~\ref{vsn-affy}.
