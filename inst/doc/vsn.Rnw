...
input, the function \Rpackage{vsn} takes the raw intensity measurements
...
the whole data are transformed by a variance-stabilizing transformation.  
After this, systematic array- or dye-biases should be removed, and the 
variance should be approximately independent of the mean intensity.  
This is useful for subsequent analyses such as hypothesis tests, ANOVA
modeling, clustering, or classification that assume that the variance
is the same for all observations\footnote{% 
  Note that \Rpackage{vsn} only addresses the dependence of the variance
...
biologically highly similar samples, one labeled in green (Cy3), one
...
along the $x$-axis, which is in many cases useful for the
visualization.  A plot in which the $x$-axis shows the average
intensities themselves is obtained by calling the \Rfunction{plot}
command with the argument \Rfunarg{ranks=FALSE}.
...
iterations may be increased through the parameter \Rfunarg{iter}.
...
class \Rclass{MIAME}, and may contain annotation (or ``metadata'')
pertinent to the experiment represented by the object. For an exprSet
with $d$ columns, \Robject{prep\$vsnParams} is a numeric vector of
length $2d$. Its elements $1,\ldots,d$ contain the additive
calibration and transformation parameters $a_i$, its elements
$d+1,\ldots,2d$ the multiplicative ones. Compare the above numbers
with the final values in Fig.~\ref{vsn-nkid-iter}.

If $y_{ki}$ is the matrix of uncalibrated data, with $k$ indexing the
rows and $i$ the columns, then the calibrated data $y_{ki}'$ is
obtained through scaling by $b_i$ and shifting by $a_i$:
\begin{equation}\nonumber
y_{ki}' = a_i+b_i y_{ki}
\end{equation}
...
@
The line \Rexpression{t2[abs(t2)>1e30] <- NA} is there to avoid a bug in \Rpackage{multtest}.
...
\Rfunction{sagmbAssess} that can be used to generate simulated data and
...
for (i in seq(along=de)) {
  ...
  res <- array(1, dim=c(length(n), nrrep, length(de)))
}
@
% $ for ESS
...
@
{\bf Note:} The code has been deactivated since it takes a long time
to run.  In a later version of the vignette (soon!), this section will
be branched out into a separate Sweave file, where CPU runtime is not
so much of a concern.

\myincfig{vsn-sagmb-show}{\textwidth}{Estimation error for the
transformation: the root mean squared difference between true and
estimated transformed data, as a function of the number of genes
$n$. If \Rfunction{vsn} works correctly, the estimation error should
decrease roughly as $n^{-1/2}$.}
...
\Rfunction{vsn} assumes that only a minority (less than half) of genes
on the arrays is detectably differentially transcribed across the
...
\Rfunarg{lts.quantile} from its default value of 0.5 to a value between
0.5 and 1.
