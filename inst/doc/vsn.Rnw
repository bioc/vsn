...
(iv) summarizing.
\Rfunction{vsn} addresses the calibration and transformation. We can use 
the function \Rfunction{expresso} to run the whole process in one go:
...
Here, we have ignored the MM values and used \Rfunction{medianpolish} for
...
The resulting plots are shown in Fig.~\ref{vsn-affy}.

\myincfig{vsn-affy3}{\textwidth}{\Rfunction{normalize.AffyBatch.vsn} example}
