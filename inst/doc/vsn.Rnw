...
plot(exprs(ny), pch = ".")
@
\myincfig{vsn-ny1-scp}{85mm}
The result is shown in Fig.~\ref{vsn-ny1-scp}.
...
\end{equation}
<<ny1-sdmean,include=FALSE,fig=TRUE,width=8,height=4,results=hide>>=
par(mfrow=c(1,2))
plot(ny, ranks=TRUE)
plot(ny, ranks=FALSE)
@
\myincfig{vsn-ny1-sdmean}{\textwidth}
Such a plot is shown in Fig.~\ref{vsn-ny1-sdmean}.
...
%%%--------iterations
<<ny1-iter,include=FALSE,fig=TRUE,width=8,height=4,results=hide>>=
par(mfrow=c(1,2))
plot(ny, what="offsets")
plot(ny, what="factors")
@
\myincfig{vsn-ny1-iter}{0.75\textwidth}
The plots in Fig.~\ref{vsn-ny1-iter}
show the values of the estimated calibration and variance 
...
The histogram is shown in the left panel of Fig.~\ref{vsn-ny1}.
%%%-------histogram of M
<<ny1-M,include=FALSE,fig=TRUE,results=hide>>=
M <- ny@h[,2] - ny@h[,1] 
hist(M, breaks=50)
@
\begin{figure}[htbp]
  \begin{center}
    \includegraphics[width=0.45\textwidth]{vsn-ny1-M}
    \includegraphics[width=0.45\textwidth]{vsn-ny2-sdmean}
     \caption{\label{vsn-ny1}}
   \end{center}
\end{figure}
...
<<ny2-sdmean,include=FALSE,fig=TRUE,results=hide>>=
...
(see the right panel of Fig.~\ref{vsn-ny1}). 
...
8 samples:
<<ny2-M,include=FALSE,fig=TRUE,width=8,height=4,results=hide>>=
refrs <- (1:8)*2-1
samps <- (1:8)*2
...
abline(h=0, col="red")
@
\myincfig{vsn-ny2-M}{\textwidth}
Fig.~\ref{vsn-ny2-M} shows the analagon to the $M$-vs-$A$-plots described in 
reference~\cite{Dudoit578}. Note that in the left scatterplot, 
there is a cloud of points at low intensities 
...
\newblock Manuscript, \code{http://www.dkfz.de/abt0840/whuber}, 2002.
