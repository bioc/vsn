...
data processing routines of \code{affy}. 
Note that the runtime of \code{vsn} is approximately proportional 
to the number of chips times the number of probes, and may be large.

The following example should work with \code{affy} <= 1.2.2.
<<affy1,eval=FALSE>>=
## library(affy)
## data(Dilution)
...
\code{affy} > 1.2.2 uses the class \code{AffyBatch} to store probe 
intensities. At the time of writing of this document (5 Nov 2002), I have not yet 
been able to test this, but a suitable wrapper should roughly look like this:
<<affy2>>=
