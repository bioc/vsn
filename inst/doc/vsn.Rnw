...
%------------------------------------------------------------ 
\Rpackage{vsn} is a method to preprocess DNA microarray intensity data. As
input, the function \Rpackage{vsn} takes the raw intensity measurements from
the DNA probes on a series of microarrays. The data are calibrated by a
suitable set of affine transformations, then the whole data are transformed by
a variance-stabilizing transformation.  After this, systematic array- or
...
hypothesis tests, ANOVA modeling, clustering, or classification work better,
or are easier to use, if the variance of the data is (roughly) the same for
all observations\footnote{Note that \Rpackage{vsn} only addresses the
dependence of the variance on the mean intensity. There may be other factors
influencing the variance, such as gene-inherent properties, or changes of the
tightness of transcriptional control in different conditions.  If necessary,
these need to be addressed by other methods.}.  Differences between the
transformed values are the so-called "generalized log-ratios".  If both
numerator and denominator are well above background, generalized log-ratios
coincide with the usual log-ratios:
$h(x_i)-h(x_j)\approx\log(x_i)-\log(x_j)=\log(x_i/x_j)$ if $x_i,
x_j\gg0$~\cite{HuberISMB2002,HSG2002}. In contrast to log-ratios, they remain
well-defined and statistically meaningful if $x_i$ or $x_j$ are close to zero.

%------------------------------------------------------------
\section{The data}
%------------------------------------------------------------ 
The prefered input type for \Rpackage{vsn} are objects of class
\Rclass{exprSet}. It is also possible to pass matrices, data frames
with numeric columns only, and objects of class \Rclass{marrayRaw}.
Furthermore, \Rpackage{vsn} can be used as a normalization method in
the function \Rfunction{expresso} in the package \Rpackage{affy}
(cf.~Section~\ref{affy}).

To load intensity data from your own experiments, you can use the
function \Rfunction{read.table}, the \Rfunction{read}-functions from
the package \Rpackage{marrayInput}, or \Rfunction{ReadAffy} from the
package \Rpackage{affy}.
...
data(kidney) 
@ 
The two columns of the matrix \Rexpression{exprs(kidney)} contain the
green and red intensities, respectively. Let's try out \Rfunction{vsn} on
...
data}
\Rfunction{vsn} returns the transformed intensities in an object of
class \Rclass{exprSet}. Its slot \Rexpression{exprs} is a matrix of the
...
stabilization, there is the function \Rfunction{meanSdPlot}. For each
...
consequence of inadequate prior data preprocessing (see
Section~\ref{sec.qc}). The rank ordering distributes the data evenly
along the $x$-axis. A plot in which the $x$-axis shows the average
intensities themselves is obtained by calling the \Rfunction{plot}
command with the argument \Rexpression{ranks=FALSE}.

The parameter estimation in \Rfunction{vsn} works in an iterative
manner.  To verify that the iterations have converged, you can call
the function \Rfunction{vsnPlotPar}.
...
iterations may be increased through the parameter \Rexpression{iter}.  It
...
Sometimes, it is better to consider \textit{stratified calibration}. For
spotted arrays, the stratification of the probes may be according to, for
example, print-tip~\cite{Dudoit578} or PCR-plate~\cite{HSG2002}. For
oligonucleotide arrays, it may be useful to stratify the probes by
physico-chemical properties, e.\,g.\, to assume that probes of different
sequence composition attract systematically different levels of unspecific
background signal.
...
@
% $ for ESS
The \Rexpression{description} slot of an \Rclass{exprSet} is an object of
...
\Rexpression{prep\$vsnParams} is a numeric array with dimensions $(n_s, d, 2)$. 
\Rexpression{prep\$vsnParams[s, i, 1]} is what was called $a_{si}$ in
Eqn.~(\ref{eq.transf}), and
\Rexpression{prep\$vsnParams[s, i, 2]} is $b_{si}$. Compare the 
numbers printed above with the final values in Fig.~\ref{vsn-nkid-iter}.

%---------------------------------------------------------------------
\subsection{Calibration with strata}
\label{sec.strata}
%---------------------------------------------------------------------
\textit{to be written} ... see help page of vsn and vsnh for more
details.
...
@
We can again call \Rfunction{vsn} on this data
...
pData(lymphoma)
@
The 16 columns of the \Rexpression{lymphoma} object contain the red and
...
\Rfunction{vsn} on all of them at once:
...
compare the obtained values against a known truth. This can be done in
controlled spike-in experiments and in dilution series, which allow to
systematically assess the performance of the methods at different
biologically relevant spike-in concentrations. Like any statistical
method, one can make different choices with respect to the trade-off
between bias and variance~\cite{Affycomp}.
...
log-ratios for slide 1 from \Rfunction{vsn}, $y$-axis -- log-ratios
...
the 4 with DLCL. The $t$-statistics from \Rfunction{vsn} are larger,
...
par(mfrow=c(1,2))
library(multtest)
plot(M[,1], mn@maM[,1], xlab="M (vsn)", ylab="M (global median)", main="slide 1", pch=".")
...
\Rfunction{vsn} for the calibration and transformation of the probe
...
three steps:
\begin{enumerate}
\item \label{joinpmmm} combining the perfect match (PM) and mismatch 
  (MM) intensities into one number per probe
\item \label{calibrate} calibrating 
\item \label{transform} transforming
\item \label{summarize} summarizing
\end{enumerate}
\Rfunction{vsn} addresses steps \ref{calibrate} and \ref{transform}. For
steps \ref{joinpmmm} and \ref{summarize}, we follow the RMA method. The 
resulting plot is shown in Fig.~\ref{vsn-affy}.
...
abline(h=0, col="red")
@
\myincfig{vsn-affy}{\textwidth}{\Rfunction{normalize.AffyBatch.vsn} example}

%---------------------------------------------------------
\section{Quality control}\label{sec.qc}
%---------------------------------------------------------
\Rfunction{vsn} makes some assumptions about your data that need to
...
First, \Rfunction{vsn} assumes that the measured signal $y_{ik}$
...
\paragraph{Print-tip and PCR effects.} In cDNA microarray data, systematic
patterns have been observed that are associated with the print-tips
used for the spotting of DNA, and with the microtitre plates used to
store and amplify the DNA. Possibly, these effects could be included
in~(\ref{assumption}) by replacing $a_i$ and $b_i$ by print-tip and/or
plate specific coefficients, but presently \Rfunction{vsn} does not
include this option.
...
qualities of DNA amplification or printing. \Rfunction{vsn} cannot be
...
\paragraph{Variance.} A further assumption that \Rfunction{vsn} makes is that 
...
\Rfunction{vsn} assumes that only a minority (less than half) of genes on the
...
by increasing the parameter \Rexpression{lts.quantile} from its default value
...
cannot be used with \Rfunction{vsn}. For Affymetrix data, we recommend
to use \Rfunction{vsn} on the probe intensities from the "CEL
...
\newblock http://affycomp.biostat.jhsph.edu
(submitted to \textit{Bioinformatics}, 2003).
