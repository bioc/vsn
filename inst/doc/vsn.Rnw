...
\includegraphics[width=0.8\textwidth]{vsn-nkid-scp}  
...
%
\begin{figure*}[t]\begin{center}
\includegraphics[width=0.8\textwidth]{vsn-nkid-sdmean}  
...
%
\begin{figure*}[t]\begin{center}
\includegraphics[width=0.8\textwidth]{vsn-lym-M}  
\caption{\label{vsn-lym-M}%
Mean-difference plots for two slides from the lymphoma example data.}
\end{center}\end{figure*}
%
Fig.~\ref{vsn-lym-M} shows the resulting $M$-vs-$A$-plots~\cite{Dudoit578}
...
%
<<figaffy,include=FALSE,fig=TRUE,width=8,height=8>>=
par(mfrow=c(2,2), pch=".")
w = c(1,3)
...
%
\begin{figure*}[t]\begin{center}
\includegraphics[width=0.8\textwidth]{vsn-figlimma}  
...
\end{center}\end{figure*}
%
<<figlimma,fig=TRUE,include=FALSE,width=8,height=4>>=
par(mfrow=c(1,2))
...
The \Rfunction{justvsn} method  allows to subtract local 
background estimates that are stored in the \Robject{Rb} and \Robject{Gb}
slots of the incoming \Rclass{RGList}. To demonstrate this, we construct
an  \Rclass{RGList} object \Robject{lymRGwbg},
%
<<makebg>>=
...
@ 
%
In reality, of course, we hope that the values in 
\Robject{Rb} and \Robject{Gb} contain some useful estimates of 
systematic trends in the measured data, not just random noise.
We can call \Rfunction{justvsn} on it 

%
<<justvsnwbg, results=hide>>=
#fitwibg = vsn2(lymRGwbg, backgroundsubtract=TRUE)
@ 
%
%fitnobg = vsn2(lymRG)
FIXME

Note that you do need to explicitely set
\Robject{backgroundsubtract=TRUE}, since the default for this
parameter is \Robject{FALSE}
...
library("marray")
mr <- new('marrayRaw', 
          maGf=exprs(lymphoma)[,iref], 
          maRf=exprs(lymphoma)[,ismp],
