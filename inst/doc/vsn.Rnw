...
why there is the function \Rfunction{justvsn}. The two-step protocol
is useful, for instance, when you want to fit the parameters on a
...
@ 
%
Beside \Rclass{ExpressionSet}s, there are also methods for handling matrices, 
...
afkid = vsn:::trsf2log2scale(log2(fit@par[,,1] + exprs(kidney) * rep(fit@par[,,2], each=nrow(kidney))), 
 fit@par)

par(mfrow=c(1,3))
Av = rowMeans(exprs(ynorm))
Aq = rowMeans(afkid)
Mv = t(diff(t(exprs(ynorm))))
Mq = t(diff(t(afkid)))

plot(Av, Mv, pch=".", ylim=c(-1,1)*5, col=ifelse(is.na(Aq), "darkorange", "black")); abline(h=0, col="blue")
plot(Aq, Mq, pch=".", ylim=c(-1,1)*5); abline(h=0, col="blue")

rv = rank(Av)
matplot(rv, cbind(Mv, Mq), pch=".")
...
%------------------------------------------------------------
\section{The relationship between vsn and background correction} 
%------------------------------------------------------------
