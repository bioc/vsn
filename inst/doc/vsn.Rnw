...
vsnwarn = 2
oopt = options(digits=3, warn=vsnwarn)
...
Furthermore, \code{vsn} can be used as a normalization methods in the
...
biologically identical samples, one labeled in green (Cy3), one in red
(Cy5), were hybridized.
% begin code ----------------------------------------
...
this example data. In Fig.~\ref{vsn-nkid-scp} you can see the
scatterplot of the calibrated and transformed data. For comparison,
the scatterplot of the raw intensities with double-logarithmic axes
scaling is also shown.
% begin code ------------------------------------------------------
...
plot(exprs(kidney), main = "raw", pch = ".", log="xy")
...
@
\myincfig{vsn-nkid-sdmean}{\textwidth}
% end code ------------------------------------------------------------
...
@
\myincfig{vsn-nkid-iter}{0.75\textwidth}
% end code ---------------------------------------------------------
...
hist(M, breaks=50)
@
\myincfig{vsn-nkid-histM}{0.5\textwidth}
% end code -----------------------------------------------------------
...
by 50 and a scale that differed by a factor of $1/2$:
<<nkid-calib2>>=
bkid = kidney
exprs(bkid)[,"red"] = 0.5 * (50+exprs(bkid)[,"red"])
@
...
options(warn=-1)
@
<<nkid-calib4,fig=TRUE,width=8,height=4>>=
preproc(description(nbkid))$vsnParams
par(mfrow=c(1,2))
plot(exprs(nbkid), main = "vsn", pch = ".")
plot(exprs(bkid),  main = "raw", pch = ".", log="xy")
@
\myincfig{vsn-nkid-calib4}{0.75\textwidth}
<<nkid-calibnowarn,echo=FALSE,results=hide>>=
options(warn=vsnwarn)
@
The factor for the red channel is now twice as large as before, and 
also the offset has decreased by amount of $\approx 50 \times b_2$.
...
in the table. Thus, the CH1 intensities are in columns $1:8$, the 
CH2 intensities in columns $9:16$.
...
meanSdPlot(lym)
@
\myincfig{vsn-lym-sdmean}{0.5\textwidth}
%------------------------------------------------------------
