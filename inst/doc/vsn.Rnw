...
Ms[,,1] <- M
library(marrayNorm)
if(FALSE) {  ## does not work presently
...
par(oldpar)
@
\myincfig{vsn-affy}{0.75\textwidth}{\code{normalize.AffyBatch.vsn} example}

%------------------------------------------------------------------------
\section{Verifying and assessing the performance of vsn with simulated data}
\label{sagmb}
%------------------------------------------------------------------------
