...
Usually, you will produce from your own data them using the
...
@ 
%
Here is what happens here: the \Rfunction{justvsn} method 
uses the package \Rpackage{convert} to coerce its argument,
which is of class \Rclass{RGList}, 
into an \Rclass{NChannelSet}, and then passes this on to 
...
quality of metadata that is in an \Rclass{RGList}, and due to the 
many differences in the implementation of these classes,
...
stopifnot(identical(arrayID, lymphoma$name[wg])) ## just to make sure :)

sampleType = sub("-.*", "", lymphoma$sample) ## sample number suffix
...
@ 
%
Now let's combine the red and green values from each array
...
The result is shown in Figure~\ref{vsn-nkid-calib4}.
