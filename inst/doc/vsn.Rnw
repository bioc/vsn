...
@ 
%
The scatterplots produced by the code below are compared in \figphrase{vsn-affy}.
%
<<figaffy1,echo=FALSE>>=
...
%---------------------------------------------------------------
\subsection{Comparison of VSN's and RMA's background correction} 
\label{sec:affybg}
%---------------------------------------------------------------
Let us use the function \Rfunction{bg.correct.rma} in the
\Rpackage{preprocessCore} package to explore the background correction
of RMA with that of VSN.
%
<<affybg1, results=hide>>=
library("preprocessCore")
rawData = pm(Dilution)
bg_rma = rma.background.correct(rawData)
@ 

The corresponding call to \Rfunction{vsn2} is as follows. We use the
\Robject{subsample} argument to speed up computations, then the function \Rfunction{exprs}
to extract the matrix of expression values, and apply the exponential transformation
to reverse the logarithmic transformation that is implicit in 
\Rfunction{vsn2}.
%
<<affybg2, results=hide>>=
fit = vsn2(rawData, subsample=4e4)
bg_vsn = 2^exprs(fit)
@ 
<<figaffybg1,echo=FALSE>>=
openBitmap("affybg", cols=1, rows=1)
@ 
Finally we can plot the data before and after background correction,
on a double-logarithmic scale. 
%
<<figaffybg2,results=hide>>=
colors = c("black", "blue")
rsel = sample(nrow(bg_rma), 3000)
matplot(rawData[rsel,1],
  cbind(bg_vsn[rsel,1], 
        bg_rma[rsel,1]),
  log="xy", pch=".", col=colors,
  main="background correction", 
  xlab="before", ylab="after")
legend("topleft", legend=c("vsn", "rma"), 
       fill=colors)
abline(a=0, b=1, col="#808080d0")
@ 
<<figaffybg3,echo=FALSE,results=hide>>=
dev.off()
@

\begin{figure*}[tb]\begin{center}
\includegraphics[width=0.45\textwidth]{vsn-affybg}  
\caption{\label{vsn-bg}%
Comparison of VSN and RMA background correction.  
The straight line corresponds to the identity line.}
\end{center}\end{figure*}
...
xoff = cc = 50
ymat = cbind(log2.na(x)*log(2), log(x+sqrt(x^2+cc^2))-log(2), log2.na(x+xoff)*log(2))
...
legend(x = par("usr")[2], y = par("usr")[3], 
       legend = c(expression(y = log(x)), expression(y = glog[c](x)), expression(y = log(x+x[off]))), 
...
\begin{figure}[tb]
\begin{center}
\includegraphics[width=0.45\textwidth]{vsn-figgraph}
\caption{\label{vsn-figgraph}Graphs of the functions $y=\log(x)$,
  $y=\glog_c(x)=\log(x+\sqrt{x^2+c^2})-\log2$, and
  $y=\log(x+x_{\mbox{\scriptsize off}})$, where $c=\Sexpr{cc}$ and
