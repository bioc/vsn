...
\Rexpression{e2} one with calibrated and \textit{glog}-transformed data.
\Rexpression{e1} can also be a matrix, a data frame with numeric columns only,
...
natural logarithm for large values (compared against the background noise),
but is less steep for smaller values.  Differences between the transformed
values are the \textit{\textbf{generalized log-ratios}}. For example,
...
possible to stratify the transformations within columns; this is discussed in
Section~\ref{sec.strata}}, then the whole data are transformed by a
