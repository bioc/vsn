...
variance stabilisation, this should be approximately a horisontal
...
artificial and were probably of higher quality than those in some
...
j = "CLL-13"
boxplot(A[,j]~spotrow, data=arrayGeometry, 
   col=colours, main=j, 
   ylab="A", xlab="spotrow")
@ 
%
\myincfig{vsn-lymquscp}{0.45\textwidth}{$M-A$-plot
of the data from array \Sexpr{j}. Dots coloured in orange
are from rows 22 and 23.}
...
underlying stratifying factor, in our case this would be presumably 
be the row within sector. However, it assumes that the stratifying factor is 
strongly confounded with $A$ (e.\,g.,\ in the current example, almost
all data points with very low $A$ values came from rows 22 and 23,
and few of the data points from these rows had high $A$-values).


\subsection{Some more on stratifying factors such as
