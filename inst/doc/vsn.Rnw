...
\Rclass{AffyBatch} is straightforward.
%
<<showcode>>=
...
methods than RMA are encouraged to take this code and modify it
accordingly, for example, by replacing the call to \Rfunction{rma}
with one to \Rfunction{mas5}.
