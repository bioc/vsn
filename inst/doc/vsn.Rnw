...
on a double-logarithmic scale. The result is shown in \figphrase{vsn-affybg}.
The RMA background correction is more aggressive.
%
...
\caption{\label{vsn-figgraph}Graphs of the functions $y=\log_2(x)$,
