...
estimate.
a) normalised and transformed with VSN, 
...
command with the argument \Robject{ranks=FALSE}; but this tends to be
less useful and hence is not the default.
...
emphasis on \emph{precision} of the expression estimate, for the price
