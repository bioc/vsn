...
which makes this assumption roughly true makes the use such methods
more appropriate.

Note, however, that \Rpackage{vsn} only addresses the dependence of
...
%------------------------------------------------------------
\section{Running \Rpackage{vsn} on data from a single two-colour array} 
%------------------------------------------------------------
...
%% RB 20.3.2007:
%% The phrase "the standard deviation" carries a pair of assumptions that
%% you haven't made explict: that all/most features (i) really do have a
%% common within-class standard deviation, and (ii) that all/most
%% features have the same mean in both classes, so that a single
%% $\hat{\mu}_k$ in (1) is appropriate. It would be nice to make these
%% clearer somewhere. The whole point of VSN, afterall, arises from
%% these.
After variance stabilisation, this should be
...
%------------------------------------------------------------
\section{Running \Rpackage{vsn} on data from multiple arrays 
...
<<lym-sdmean2,include=FALSE,fig=TRUE,width=4,height=4.5>>=
meanSdPlot(lym)
@
...
%---------------------------------------------------------
\section{Running \Rpackage{vsn} on Affymetrix genechip data} \label{affy}
%---------------------------------------------------------
...
The \Rpackage{vsn} method addresses steps (i)--(iii).
...
%---------------------------------------------------------
\section{Running \Rpackage{vsn} on \Robject{RGList} objects} \label{limma}
%---------------------------------------------------------
...
%
<<strata, results=hide>>=
EconStr = justvsn(lymRG[,1], strata=pinId)
@
%
...
%-------------------------------------------------------------------
\section{Assessing vsn and how to compare to other methods}
%-------------------------------------------------------------------
...
the 4 with DLCL. The moderated $t$-statistics from \Rpackage{vsn} are slightly 
larger, i.\,e.\ processing with \Rpackage{vsn} results in a slightly
...
Martin Vingron, Richard Bourgon and Ulrich Mansmann.
