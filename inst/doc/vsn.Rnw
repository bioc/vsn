...
<<loadotherpackages,echo=FALSE,results=hide>>=
library(affy)
library(marrayNorm)
library(multtest)

vsnwarn <- 0
oopt <- options(warn=vsnwarn)
...
@ 
% end code -----------------------------------------
The 2 columns of the matrix \code{exprs(kidney)} object contain the
...
% begin code ------------------------------------------------------
<<nkid-scp,echo=FALSE,results=hide,include=FALSE,fig=TRUE,width=8,height=4>>=
par(mfrow=c(1,2))
if(!exists("nkid")) 
  nkid <- vsn(kidney)
...
@
<<nkid-scp-show,eval=FALSE>>=
nkid <- vsn(kidney)
plot(exprs(nkid), main = "vsn", pch = ".")
plot(log.na(exprs(kidney)), main = "raw", pch = ".")
@
\myincfig{vsn-nkid-scp}{0.75\textwidth}{Scatterplots of the kidney 
example data}
% end code ------------------------------------------------------------
...
@
\myincfig{vsn-nkid-iter}{0.75\textwidth}{Iteration trajectory of the 
...
iterations may be increased through the parameter \code{iter}, and/or
\code{lts.quantile} may be decreased. It could also indicate a
data quality problem, see Section~\ref{sec.qc}.
...
y_{ki}' = a_i+b_i y_{ki}
\end{equation}
Now suppose the kidney example data were not
that well measured, and the red channel had a baseline that was shifted 
by 500 and a scale that differed by a factor of $0.25$:
<<nkid-calib2>>=
bkid <- kidney
exprs(bkid)[,"red"] <- 0.25 * (500+exprs(bkid)[,"red"])
@
We can again call \code{vsn} on this data
<<nkid-calib3,echo=FALSE,results=hide>>=
if(!exists("nbkid")) 
  nbkid <- vsn(bkid)
@
<<nkid-calib3-show,eval=FALSE>>=
nbkid <- vsn(bkid)
@
<<nkid-calibnowarn,echo=FALSE,results=hide>>=
options(warn=-1)
@
...
@
\myincfig{vsn-nkid-calib4}{0.9\textwidth}{Scatterplots}
<<nkid-calibnowarn,echo=FALSE,results=hide>>=
options(warn=vsnwarn)
@
The factor for the red channel is now about four times as large as before.
...
%------------------------------------------------------------
<<lym-sdmean,echo=FALSE,results=hide,include=FALSE,fig=TRUE>>=
if(!exists("lym")) 
  lym <- vsn(lymphoma, verbose=FALSE)
meanSdPlot(lym)
@
<<lym-sdmean-show,eval=FALSE>>=
lym <- vsn(lymphoma, verbose=FALSE)
meanSdPlot(lym)
@
\myincfig{vsn-lym-sdmean}{0.5\textwidth}{Standard deviation 
versus rank of the mean for the lymphoma example data}
%------------------------------------------------------------
This calculation may take a while. Again, Fig.~\ref{vsn-lym-sdmean} 
...
between CLL and DLCL. The comparison relies on the idea that for 
...
<<Ms-plot,results=hide,fig=TRUE,include=FALSE>>=
...
@
\myincfig{vsn-Ms-plot}{80mm}{Generalized log-ratio from \code{vsn} ($x$-axis)
versus the log-ratio after global median normalization ($y$-axis)}
...
\myincfig{vsn-fdr3}{80mm}{False Discovery Rate}
<<fdr1,results=hide>>=
library(multtest)
...
@
<<fdr3,fig=TRUE,include=FALSE,results=hide>>=
...
the probe sets. See the example for \code{normalize.AffyBatch.vsn}.
The resulting plot is shown in Fig.~\ref{vsn-affy}.
<<affy,include=FALSE,results=hide,fig=TRUE,width=7,height=7>>=
example(normalize.AffyBatch.vsn)
@
\myincfig{vsn-affy}{0.75\textwidth}{\code{normalize.AffyBatch.vsn} 
example}
...
\label{sagmb}
%---------------------------------------------------------
There are two functions \Rfunc{sagmbSimulateData} and \Rfunc{sagmbAssess}
that can be used to generate simulated data and assess the difference between 
the 'true' and 'estimated' data calibration and transformation by vsn. An 
example is shown in the code chunk below. Reference~\cite{HuberSAGMB2003} 
describes in more detail
\begin{itemize}
\item the simulation model
\item the assessment strategy
\item a comprehensive suite of assessments with respect to the number
of probes $n$, the number of arrays $d$, the fraction of
differentially expressed genes $de$, and the fraction of up-regulated
genes $up$.
\end{itemize}
<<sagmb,include=FALSE,results=hide,fig=TRUE,width=7,height=4>>=
par(mfrow=c(1,2))
...
for (i in seq(along=de)) {
  res <- matrix(NA, ncol=nrrep, nrow=length(n))
  for (k in seq(along=n)) {
    for (r in 1:nrrep) {
      sim       <- sagmbSimulateData(n[k], d, de=de[i], up=0.5)
      ny        <- vsn(sim$y)  
      res[k, r] <- sagmbAssess(exprs(ny), sim)
    }
  }
  matplot(n, res, pch=20, log="xy", col="#909090", main=paste("de=", de[i]))
  lines(n, rowMeans(res), col="blue")
}
@
...
\code{lts.quantile} and must be less than or equal to $1/2$.  
By default, \code{lts.quantile}=0.5, that is, the maximum.
