...
\arsinh\left(\frac{y_{ki}}{f(b_i)}+a_i\right) = \mu_k + \varepsilon_{ki}
\end{equation}
where $\mu_k$, for $k=1,\ldots,n$, and $a_i$, $b_i$, for $i=1,\ldots,d$
are real-valued parameters, $f$ is a function $\mathbb{R}\to\mathbb{R}$ (see below), 
...
Examples for $f$ are $f(b)=b$ and $f(b)=e^b$. The former is the most obvious choice;
in that case we need to require $b_i>0$ in order to make sure that the denominator
on the LHS of Equation~(\ref{eq:model}) is always positive.
The choice $f(b)=e^b$ assures a positive denominator for all $b\in\mathbb{R}$, and as it 
turns out, simplifies some of the computations.

In the following calculations, I will also use the notation
\begin{eqnarray}
h \equiv h(y,a,b) &=& \arsinh\left(\frac{y}{f(b)}+a\right)\\
Y \equiv Y(y,a,b) &=&        \left(\frac{y}{f(b)}+a\right).
\end{eqnarray}
...
\begin{eqnarray}
\frac{dh}{dy}&=&
\frac{1}{f(b)\sqrt{1+\left(\frac{y+a}{f(b)}\right)^2}}=
\frac{1}{f(b)\sqrt{1+Y^2}},\\
\frac{\partial h}{\partial a}&=&\frac{1}{\sqrt{1+Y^2}},\\
\frac{\partial h}{\partial b}&=&-\frac{Y}{\sqrt{1+Y^2}}\cdot\frac{f'(b)}{f(b)}\\
\frac{\partial Y}{\partial a}&=&1\\
\frac{\partial Y}{\partial b}&=&-Y\cdot\frac{f'(b)}{f(b)}.
\end{eqnarray}
Note that for $f(b)=b$, we have $\frac{f'(b)}{f(b)}=\frac{1}{b}$,
and for $f(b)=e^b$,  $\frac{f'(b)}{f(b)}=1$.
...
+\log\left(f(b_i)\sqrt{1+Y_{ki}^2}\right)\right)\\
...
\frac{f'(b_i)}{f(b_i)} \left(n
-\sum_{k=1}^n \left( \frac{h(y_{ki})-\mu_k}{\sigma^2}
+\frac{Y_{ki}}{\sqrt{1+Y_{ki}^2}}\right)
\cdot\frac{Y_{ki}}{\sqrt{1+Y_{ki}^2}}\right)
\nonumber\\
&=&\frac{f'(b_i)}{f(b_i)} \left(n
-\sum_{k=1}^n 
\left(\frac{r_{ki}}{\sigma^2}+B_{ki}\right)B_{ki}\right)
\label{eq:ddbnll}
\end{eqnarray}
...
\end{eqnarray}
These variables are also used in the C code to simplify 
...
\log\left(f(b_j){\sqrt{1+Y_{kj}^2}}\right).
\end{equation}
...
-\frac{2}{nd}\cdot\frac{f'(b_i)}{f(b_i)} 
...
\frac{\partial \hat{\sigma}^2}{\partial a_i} 
+\sum_{k=1}^n A_{ki}B_{ki}\nonumber\\
&=&\sum_{k=1}^n 
\left(\frac{r_{ki}}{\hat{\sigma}^2}+B_{ki}\right) 
A_{ki}\label{eq:ddanpll}\\
\frac{\partial}{\partial b_i}(-PLL) &=&
\frac{nd}{2\hat{\sigma}^2}\cdot 
\frac{\partial \hat{\sigma}^2}{\partial b_i} 
+ \frac{f'(b_i)}{f(b_i)} \left(n
- \sum_{k=1}^n B_{ki}^2\right)\nonumber\\
&=& \frac{f'(b_i)}{f(b_i)} \left(n
- \sum_{k=1}^n 
\left(\frac{r_{ki}}{\hat{\sigma}^2}+B_{ki}\right) \right)
...
\frac{\partial}{\partial a_i}(-PLL) &=&
\sum_{k=1}^n 
\left(\frac{r_{ki}}{\hat{\sigma}^2}+B_{ki}\right)A_{ki}
\end{eqnarray}
...
\begin{eqnarray}
\frac{\partial}{\partial b_i}(-LL_i) &=&
\frac{f'(b_i)}{f(b_i)}\left(n-\sum_{k=1}^n 
\left(\frac{r_{ki}}{\sigma^2}+B_{ki}\right)B_{ki}\right)\\
%
\frac{\partial}{\partial b_i}(-PLL) &=&
\frac{f'(b_i)}{f(b_i)}\left(n-\sum_{k=1}^n 
\left(\frac{r_{ki}}{\hat{\sigma}^2}+B_{ki}\right)B_{ki}\right)
\end{eqnarray}
