...
was presented in the initial publication on \Rpackage{vsn}~\cite{HuberISMB2002} 
and implemented in versions 1.X of the package.
...
\]
and the integration is over a volume element of $y$-space. The likelihood is
\[
