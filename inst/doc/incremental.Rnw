...
are real-valued parameters ($b_i>0$), and $\varepsilon_{ki}$ are 
i.i.d.\ Normal with mean 0 and variance $\sigma^2$. $y_{ki}$ are the data.
In applications to $\mu$array data, $k$ indexes the features and $i$ the 
arrays and/or colour channels.

In the following calculations, I will also use the notation
\begin{eqnarray}
h \equiv h(y,a,b) &=& \arsinh\frac{y+a}{b}\\
Y \equiv Y(y,a,b)  &=& \frac{y+a}{b}.
\end{eqnarray}
...
\frac{\partial Y}{\partial a}&=&\frac{1}{b}\\
\frac{\partial Y}{\partial b}&=&-\frac{Y}{b}.
\end{eqnarray}
