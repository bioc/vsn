%\VignetteIndexEntry{Introduction to vsn (Sweave version)}
...
\begin{document}
%----------------------------------------------------------------------------------------
\title{Introduction to vsn (Sweave version)}
...
%
\begin{figure*}
\includegraphics{0-vsn-nkid-scp}  
\caption{\label{0-vsn-nkid-scp}%
...
%
\begin{figure*}
\includegraphics[width=0.49\textwidth]{0-vsn-nkid-sdmean1}  
\includegraphics[width=0.49\textwidth]{0-vsn-nkid-sdmean2}  
\caption{\label{0-vsn-nkid-sdmean}%
...
\figphrase{0-vsn-nkid-scp} you can see the scatterplot of the
...
<<nkid-sdmean1,results=hide,fig=TRUE,height=3>>=
meanSdPlot(xnorm, ranks=TRUE)
@ 
<<nkid-sdmean2,results=hide,fig=TRUE,height=3>>=
meanSdPlot(xnorm, ranks=FALSE)
@

The two plots are shown in \figphrase{0-vsn-nkid-sdmean}.  The red
...
\figphrase{0-vsn-nkid-histM}.
%
<<nkid-histM,echo=FALSE,fig=TRUE,results=hide>>=
hist(M, breaks=33, col="#d95f0e")
@
\incfig{0-vsn-nkid-histM}{0.65\textwidth}{Histogram of generalised 
log-ratios \Robject{M} for the kidney example data.}

\incfig{0-vsn-lym-sdmean}{0.65\textwidth}{Standard deviation 
versus rank of the mean for the lymphoma example data}

\incfig{0-vsn-lym-M}{0.65\textwidth}{$M$-$A$-plot 
...
@ 
<<lym-sdmean,results=hide,fig=TRUE,height=3>>=
meanSdPlot(lym)
@

Again, \figphrase{0-vsn-lym-sdmean} helps to visually verify that the
...
@
%
\figphrase{0-vsn-lym-M} shows the resulting $M$-$A$-plot~\cite{Dudoit578}
...
@ 
%
The scatterplots produced by the code below are compared in \figphrase{0-vsn-affy}.
%
<<figaffy1,echo=FALSE>>=
...
\begin{figure*}
\includegraphics[width=\textwidth]{0-vsn-affy}  
\caption{\label{0-vsn-affy}%
Results of \Rfunction{vsnrma} and \Rfunction{rma} on the Dilution 
example data. Array 1 was hybridised with 
$\Sexpr{as.character(pData(Dilution)$liver[1])}\mu$g RNA from liver,
array 3 with 
$\Sexpr{as.character(pData(Dilution)$liver[3])}\mu$g of the same RNA.}
\end{figure*}
...
%
\begin{figure*}
\includegraphics[width=\textwidth]{0-vsn-figlimma}  
\caption{\label{0-vsn-figlimma}%
...
@ 
%
\figphrase{0-vsn-figlimma} shows the resulting $p$-values and
...
%--------------------------------------------------
\begin{smallfigure}
\includegraphics[width=\textwidth]{0-vsn-figstrata}  
\caption{\label{0-vsn-figstrata}%
...
using the two models, is shown in \figphrase{0-vsn-figstrata}.
%
...
%---------------------------------------------------------------------
\begin{figure*}
\includegraphics[width=\textwidth]{0-vsn-figmiss}  
\caption{\label{0-vsn-figmiss}%
...
@ 
%
Note that \Robject{p1} and \Robject{p2} would differ more if we used a
different value than \Robject{1} for the \Robject{lts.quantile}
argument in the above calls of \Rfunction{vsn2}.  This is because the
outlier removal algorithm of \Rfunction{vsn2} will, for this dataset,
identify different sets of features as outliers for \Robject{fit1} and
\Robject{fit2} and consequently the optimisation result will be
slightly different; this difference is arguably negligible compared to
...
% 
\incfig{0-vsn-figref}{0.65\textwidth}{Scatterplot of normalised 
...
@
% 
and compare the resulting values in the scatterplot shown in \figphrase{0-vsn-figref}:
...
%---------------------------------------------------------------------
\begin{figure*}
\includegraphics[width=\textwidth]{0-vsn-nkid-calib}  
\caption{\label{0-vsn-nkid-calib}%
...
The result is shown in \figphrase{0-vsn-nkid-calib}.
...
dev.off()
@ 
The result is shown in \figphrase{0-vsn-qvsn}.

\begin{figure*}
\includegraphics[width=\textwidth]{0-vsn-qvsn}  
\caption{\label{0-vsn-qvsn}%
...
<<figgraph,fig=TRUE,results=hide,echo=FALSE>>=
...
%
\begin{smallfigure}
\includegraphics[width=\textwidth]{0-vsn-figshrink}
\caption{\label{0-vsn-figshrink}The shrinkage property of the 
...
\begin{smallfigure}
\includegraphics[width=\textwidth]{0-vsn-figgraph}
\caption{\label{0-vsn-figgraph}Graphs of the functions $y=\log_2(x)$,
  $y=\glog_2(x,c)=\log_2(x+\sqrt{x^2+c^2})-1$, and
  $y=\log_2(x+x_{\mbox{\scriptsize off}})$, where $c=\Sexpr{cc}$ and
  $x_{\mbox{\scriptsize off}}=\Sexpr{xoff}$.
}
\end{smallfigure}
%
...
An illustration is shown in \figphrase{0-vsn-figshrink}.  Data were
...
%---------------------------------------------------------
\begin{figure*}
\includegraphics[width=\textwidth]{0-vsn-lymbox}  
...
@ 
%
\incfig{0-vsn-lymquscp}{0.65\textwidth}{$M$-$A$-plot of the data from
...
(\figphrase{0-vsn-lymquscp}), we see that these features not only have
...
The output of \Rfunction{sessionInfo()} on the system on which
