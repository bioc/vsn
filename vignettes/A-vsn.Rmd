...
and then apply the model to the complete set of data^[See Section \@ref(sec:spike) for details.].
Furthermore, it enables further inspection of the `fit` object, e.g., for the
...
but is less steep for smaller values. While the $\log_2$ function diverges to
negative infinity at zero, the glog$_2$ function approximates a straight line of
finite slope around 0^[See Section \@ref(sec:shrink) for a visualization.].
Differences between the transformed values are the generalised log-ratios. These 
are shrinkage estimators of the logarithm of the fold change.
The usual log-ratio is another example for an estimator^[In statistics,
the term estimator is used to denote any algorithm
...
The VSN transformation only addresses the
dependence of the variance on the mean intensity. There may be important other
factors influencing the variance, including gene-inherent properties,
sample-inherent properties such as different tightness of transcriptional control in different
conditions, or technical factors. These need to be addressed by other, more specialized models.

### Numerical stability and convergence

VSN does not work for all possible inputs. Failures may be due to a number of problems:

- A bug in the code that does the preprocessing of the data that goes in
...
- The noise or variance characteristics of the data are not suitable for the VSN mode.

So if you encounter an error message like "L-BFGS-B needs finite values of 'fn'", please check whether one of the above 
applies^[More technically: fitting the VSN model involves optimisation of a multivariate, non-quadratic likelihood.
For bad or unsuitable data, this function may be non-convex, and in particular, it may be flat in some directions of parameter space. In such cases,
the optimiser may run out into infinity, causing the mentioned error. One could contemplate a version of `vsn2` that avoids this by using regularisation.
In any case, the current behaviour alerts users to the above problems rather than simply glossing over.].
...
```{r nkid-sdmean1, fig.cap = "Standard deviation versus rank of the mean", fig.height = 4, fig.small = TRUE}
meanSdPlot(xnorm, ranks = TRUE)
```
```{r nkid-sdmean2, fig.cap = "Standard deviation versus mean", fig.height = 4, fig.small = TRUE}
...
```{r nkid-histM, fig.cap = "Histogram of generalised log-ratios for the kidney example data.", fig.small = TRUE}
...
``` 
```{r lym-sdmean, fig.cap = "Standard deviation versus rank of the mean for the lymphoma example data", fig.height = 4, fig.small = TRUE}
meanSdPlot(lym)
```
...
```{r lym-M, fig.cap = "MA-plot for slide DLCL-0032 from the lymphoma example data. A false-colour representation of the data point density is used, in addition the 100 data points in the least dense regions are plotted as dots.", fig.small = TRUE}
iref = seq(1, 15, by=2)
ismp = seq(2, 16, by=2)
...
```{r affy, fig.height=3.75, fig.wide = TRUE, fig.cap = paste0("Results of `vsnrma` and `rma` on the Dilution data. Array 1 was hybridised with ", pData(Dilution)$liver[1], "$\\mu$g RNA from liver, Array 3 with ", pData(Dilution)$liver[3], "$\\mu$g of the same RNA.")}
par(mfrow = c(1, 3))
ax = c(2, 16)
...
```{r limma, fig.wide = TRUE, fig.cap = paste("Left: histogram of p-values from the moderated $t$-test between the",  levels(lymNCS$sampR)[1], "and",  levels(lymNCS$sampR)[2], "groups on the `lymM` values. Right: M-values for the 5 genes with the smallest p-values.")}
par(mfrow=c(1,2))
...
using the two models, is shown in Figure \@ref(fig:figstrata2).
...
%
```{r miss, fig.wide = TRUE, fig.cap="Scatterplots of fitted parameters, values on the $x$-axis correspond to normalisation without missing data (`fit1`), values on the $y$-axis to normalisation with around 10% missing data (`fit2`)."}
par(mfrow=c(1,2))
for(j in 1:2){
  p1 = coef(fit1)[,,j]
  p2 = coef(fit2)[,,j]
...
```{r ref, fig.small = TRUE, fig.cap = "Scatterplot of normalised intensities after normalisation by reference ($x$-axis, `f8`) and joint normalisation ($y$-axis, `fall`). There is good agreement."}
...
where $s\equiv s(k)$ is the so-called _stratum_ for feature $k$. In the
...
\begin{eqnarray}
h_{ki} &=& \text{arsinh}(\lambda_0y_{ki}'+\alpha_0) (\#eq:vs) \\
&=& \log\left(
...
The parameters $a$ and $b$ from Equation \@ref(eq:transf)^[I
...
\begin{eqnarray}
a&=&\frac{c_2}{\sqrt{c_3}}\nonumber\\ 
e^b&=&\frac{c_1}{\sqrt{c_3}}(\#eq:errmod2trsf)
\end{eqnarray}
...
```{r nkid-calib4, warning = FALSE, fig.cap = "Scatterplot for the badly biased `bkid` data: log-log scale.", fig.small = TRUE}
plot(exprs(bkid), main = "raw", pch = ".", log = "xy")
```
```{r nkid-calib5, warning = FALSE, fig.cap = "Scatterplot for the badly biased `bkid` data: after calibration and transformation with vsn.", fig.small = TRUE}
...
```{r vsnQ1, results = "hide", fig.cap = "Scatterplot between the red and green intensities of the array of the lymphoma dataset after quantile normalisation followed by VSN variance stabilisation without calibration. ", fig.small = TRUE} 
lym_q = normalizeQuantiles(exprs(lymphoma))
lym_qvsn = vsn2(lym_q, calib="none")
plot(exprs(lym_qvsn)[, 1:2], pch=".", main="lym_qvsn")
```
```{r vsnQ2, results = "hide", fig.cap = "Comparison of the values calculated in this section, for CH1 of the first array, to those of VSN variance stabilisation with affine calibration (`lym` was computed in Section \\@ref(sec:lymphoma)).", fig.small = TRUE} 
...
```{r shrink, results = "hide", echo = FALSE, fig.cap = "The shrinkage property of the generalised log-ratio.  Blue diamonds and error bars correspond to mean and standard deviation of the generalised log-ratio $h$, as obtained from VSN, and black dots and error bars to the standard log-ratio $q$ (both base 2).", fig.small = TRUE}
...
```{r figgraph, results = "hide", echo = FALSE, fig.cap = "Graphs of the functions. $y=\\log_2(x)$, $y=$glog$_2(x,c)=\\log_2(x+\\sqrt{x^2+c^2})-1$, and $y=\\log_2(x+x_{\\text{off}})$, where $c=50$ and $x_{\\text{off}}=50$.", fig.height = 5, fig.small = TRUE}
...
An illustration is shown in Figure \@ref(fig:shrink).  Data were
...
```{r lymbox, fig.cap = "Boxplot of A values of array CLL-13 stratified by within-sector row. The features in rows 22 and 23 are all very dim."}
...
the different overall efficiencies of the two dyes^[ 
...
as in \@ref(eq:assumption1). As long as this only occurs sporadically, this 
...
assumption \@ref(eq:assumption1) or \@ref(eq:assumption2) 
...
`r Biocpkg("vsn")` package was from 2001 to 2003, and most of this document was written
