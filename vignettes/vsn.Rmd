...
plot(log2(exprs(kidney)[select, ]), main = "a) raw", pch = ".",
     asp = 1)
plot(exprs(xnorm), main = "b) vsn", pch = ".",
     asp = 1, col = ifelse(select, "black", "orange"))
```
...
```{r nkid-sdmean1, fig.height = 3, fig.width = 3, fig.small = TRUE, fig.cap = "Standard deviation versus rank of the mean"}
meanSdPlot(xnorm, ranks = TRUE)
```
```{r nkid-sdmean2, fig.height = 3, fig.width = 3, fig.small = TRUE, fig.cap = "Standard deviation versus mean"}
...
```{r nkid-histM, fig.cap = "Histogram of generalised log-ratios for the kidney example data.", fig.width = 3.5, fig.height = 3.5, fig.small = TRUE}
...
``` 
```{r lym-sdmean, fig.height = 3, fig.width = 3, fig.small = TRUE, fig.cap = "Standard deviation versus rank of the mean for the lymphoma example data"}
meanSdPlot(lym)
```
...
```{r affy, fig.width = 8, fig.height = 3, out.width = "750px", fig.cap = paste0("Results of `vsnrma` and `rma` on the Dilution data. Array 1 was hybridised with ", pData(Dilution)$liver[1], "$\\mu$g RNA from liver, Array 3 with ", pData(Dilution)$liver[3], "$\\mu$g of the same RNA.")}
par(mfrow = c(1, 3))
ax = c(2, 16)
...
```{r limma, fig.width = 8, fig.height = 3.5, fig.cap = paste("Left: histogram of p-values from the moderated $t$-test between the",  levels(lymNCS$sampR)[1], "and",  levels(lymNCS$sampR)[2], "groups on the `lymM` values. Right: M-values for the 5 genes with the smallest p-values.")}
par(mfrow=c(1,2))
