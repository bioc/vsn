---
title: "Introduction to robust calibration and variance stabilisation with VSN"
author: "Wolfgang Huber"
date: "`r doc_date()`"
package: "`r pkg_ver('BiocStyle')`"
vignette: >
  %\VignetteIndexEntry{Introduction to robust calibration and variance stabilisation with VSN}
...
\Rclass{ExpressionSet} with calibrated and $\mathop{\mathgroup\symoperators glog}\nolimits_2$-transformed
data.
...
The so-called $\mathop{\mathgroup\symoperators glog}\nolimits_2$ (short for \emph{generalised logarithm}) 
...
a $\mathop{\mathgroup\symoperators glog}\nolimits_2$ transformation whose aim is variance stabilisation.
...
```{r nkid-scp, results = "hide", dev = "png", fig.cap="Scatterplots of the kidney example data, which were obtained from a  two-color cDNA array by quantitating spots and subtracting a local background estimate. a) unnormalised and $\\log_2$-transformed. b) normalised and transformed with VSN. Panel b shows the data from the complete set of `r nrow(kidney)` spots on the array, Panel a only the `r sum(0==rowSums(exprs(kidney)<=0))` spots for which both red and green net intensities were greater than 0. Those spots which are missing in Panel a are coloured in orange in Panel b.", fig.width = 9, fig.height = 4}
par(mfrow = c(1,2))
...
```{r nkid-sdmean, results = "hide", fig.height = 3, fig.width = 3, fig.cap = "Standard deviation versus rank of the mean, and the mean, respectively."}
msd1 = meanSdPlot(xnorm, ranks=TRUE)
msd2 = meanSdPlot(xnorm, ranks=FALSE) 

library("gridExtra")
grid.arrange(ncol = 2, msd1$gg, msd2$gg)
```

The two plots are shown in \@ref{fig:nkid-sdmean}.  The red
...
```{r nkid-histM, fig.cap = "Histogram of generalised log-ratios for the kidney example data."}
...
``` 
```{r lym-sdmean, fig.cap = "Standard deviation versus rank of the mean for the lymphoma example data"}
meanSdPlot(lym)
```
Again, Figure \@ref{fig:lym-sdmean} helps to visually verify that the
...
```{r lym-M, fig.cap = "MA-plot for slide \texttt{DLCL-0032} from the lymphoma example data. A false-colour representation of the data point density is used, in addition the 100 data points in the least dense regions are plotted as dots."}
iref = seq(1, 15, by=2)
ismp = seq(2, 16, by=2)
...
Figure \@ref{fig:lym-M} shows the resulting $M$-$A$-plot~\cite{Dudoit578} for one of the arrays. 
...
The scatterplots produced by the code below are compared in Figure \ref{fig:affy}.

```{r affy, fig.cap = "Results of `vsnrma` and `rma` on the Dilution data. Array 1 was hybridised with `r as.character(pData(Dilution)$liver[1])` $\\mu$g RNA from liver, Array 3 with `r as.character(pData(Dilution)$liver[3])` $\\mu$g of the same RNA."}
ax = c(2, 16)

plot(exprs(d_vsn)[,c(1,3)], 
  main = "vsn: array 1 vs 3",
  asp=1, xlim=ax, ylim=ax, pch=".")
...
# Running VSN on \Robject{RGList} objects{#sec:limma}
...
\begin{figure*}
\includegraphics[width=\textwidth]{vsn-figlimma}  
\caption{
\end{figure*}
%
...
The return value is an \Rclass{NChannelSet}, as shown below.
```{r lymNCS}
lymNCS
``` 
%
...
Figure \@ref{fig:limma} shows the resulting $p$-values and
the expression profiles of the genes corresponding to the 
top 5 features.

```{r limma, fig.width=8, fig.cap = "Left: histogram of p-values from the moderated $t$-test between the `r levels(lymNCS$sampR)[1]` and `r levels(lymNCS$sampR)[2]` groups on the `lymM` values. Right: M-values for the 5 genes with the smallest p-values."}
par(mfrow=c(1,2))
...
(sectors) or microtitre plates. This can be done with the \Robject{strata} parameter of
\Rfunction{vsn2}.
...
```{r figstrata2, dev = "png", fig.cap = "Scatterplot of normalised and transformed intensities for the red channel of Array 1. Values on the $x$-axis correspond to normalisation without strata (`EsenzaStr`), values on the $y$-axis to normalisation with strata (`EconStr`). The different colours correspond to the 16 different strata."}
j = 1
...
%
```{r miss, width=8, fig.cap="Scatterplots of fitted parameters, values on the $x$-axis correspond to normalisation without missing data (`fit1`), values on the $y$-axis to normalisation with around 10\\% missing data (`fit2`)."}
par(mfrow=c(1,2))
for(j in 1:2){
  p1 = coef(fit1)[,,j]
  p2 = coef(fit2)[,,j]
...
Note that \Robject{p1} and \Robject{p2} would differ more if we used a different value than \Robject{1} for the \Robject{lts.quantile} argument in the above calls of \Rfunction{vsn2}.  This is because the outlier removal algorithm of \Rfunction{vsn2} will, for this dataset, identify different sets of features as outliers for \Robject{fit1} and \Robject{fit2} and consequently the optimisation result will be slightly different; this difference is arguably negligible compared to the noise level in the data.
...
```{r ref, dev = "png", fig.cap = "Scatterplot of normalised intensities after normalisation by reference ($x$-axis, `f8`) and joint normalisation ($y$-axis, `fall`). There is good agreement."}
...
\begin{eqnarray}
h_{ki} &=& \mathop{\mathgroup\symoperators arsinh}\nolimits(\lambda_0y_{ki}'+\alpha_0) \label{eq.vs} \\
&=& \log\left(
...
h_{ki} = \mathop{\mathgroup\symoperators arsinh}\nolimits\left(e^{b_{si}}\cdot y_{ki}+a_{si}\right). 
\end{equation}
Here, 
...
```{r nkid-calib, fig.cap="Scatterplots for badly biased data. Left hand side: raw data on log-log scale, right hand side: after calibration and transformation with vsn."}
...
It is possible to force $\lambda_{si}=1$ and $\alpha_{si}=0$ for all $s$ and $i$ in Equation~(\ref{eq.cal}) by setting \Rfunction{vsn2}'s parameter \Robject{calib} to \texttt{"none"}. Hence, only the global variance stabilisation transformation~(\ref{eq.vs}) will be applied, but no column- or row-specific calibration.

Here, I show an example where this feature is used in conjunction with
quantile normalisation.

```{r vsnQ, results = "hide", fig.cap = "The left panel shows the scatterplot between the red and green intensities of the array of the lymphoma dataset after quantile normalisation followed by VSN variance stabilisation without calibration. The right panel compares the values from that method, for CH1 of the first array, to that of VSN variance stabilisation with affine calibration (`lym` was computed in Section \\@ref{sec:lymphoma})."} 
...
```{r shrink, results = "hide", echo = FALSE, fig.cap = "The shrinkage property of the generalised log-ratio.  Blue diamonds and error bars correspond to mean and standard deviation of the generalised log-ratio $h$, as obtained from VSN, and black dots and error bars to the standard log-ratio $q$ (both base 2)."}
...
```{r figgraph, results = "hide", echo = FALSE, fig.cap = "Graphs of the functions $y=\\log_2(x)$, $y=\\mathop{\\mathgroup\\symoperators glog}\\nolimits_2(x,c)=\\log_2(x+\\sqrt{x^2+c^2})-1$, and $y=\\log_2(x+x_{\\mbox{\\scriptsize off}})$, where $c=\\Sexpr{cc}$ and $x_{\\mbox{\\scriptsize off}}=`r xoff`$."}
...
model~\cite{RockeDurbin2001,HuberSAGMB2003,HuberWiley2004}.  The
...
smaller intensities, we can see a \textit{variance-bias trade-off}:
...
$\mathop{\mathgroup\symoperators glog}\nolimits_2$-transformed intensities. The dataframe
...
```{r lymbox, fig.width = 8, fig.cap = "Boxplot of A values of array CLL-13 stratified by within-sector row. The features in rows 22 and 23 are all very dim."}
