...
knitr::opts_chunk$set(tidy = FALSE, cache = TRUE, autodep = TRUE,
    dev = "png", dpi = 300, message = FALSE, error = FALSE, warning = TRUE)
set.seed(0xdada)
```
...
```{r nkid-histM, fig.cap = "Histogram of generalised log-ratios for the kidney example data.", fig.width = 3.5, fig.height = 3.5, out.width = "300px"}
...
```{r lym-M, fig.cap = "MA-plot for slide DLCL-0032 from the lymphoma example data. A false-colour representation of the data point density is used, in addition the 100 data points in the least dense regions are plotted as dots.", fig.width = 4.5, fig.height = 3, out.width = "450px"}
iref = seq(1, 15, by=2)
ismp = seq(2, 16, by=2)
...
```{r affy, fig.width = 8, fig.height = 3, out.width = "750px", fig.cap = "Results of `vsnrma` and `rma` on the Dilution data. Array 1 was hybridised with `r as.character(pData(Dilution)$liver[1])` $\\mu$g RNA from liver, Array 3 with `r as.character(pData(Dilution)$liver[3])` $\\mu$g of the same RNA."}
par(mfrow = c(1, 3))
ax = c(2, 16)
...
```{r limma, fig.width = 8, fig.height = 3.5, out.width = "600px", fig.cap = "Left: histogram of p-values from the moderated $t$-test between the `r levels(lymNCS$sampR)[1]` and `r levels(lymNCS$sampR)[2]` groups on the `lymM` values. Right: M-values for the 5 genes with the smallest p-values."}
par(mfrow=c(1,2))
...
```{r figstrata2, fig.width = 4, fig.height = 4.5, out.width = "300px", fig.cap = "Scatterplot of normalised and transformed intensities for the red channel of Array 1. Values on the $x$-axis correspond to normalisation without strata (`EsenzaStr`), values on the $y$-axis to normalisation with strata (`EconStr`). The different colours correspond to the 16 different strata."}
j = 1
...
%
```{r miss, fig.width = 6, fig.height = 3.5, out.width = "600px", fig.cap="Scatterplots of fitted parameters, values on the $x$-axis correspond to normalisation without missing data (`fit1`), values on the $y$-axis to normalisation with around 10\\% missing data (`fit2`)."}
par(mfrow=c(1,2))
for(j in 1:2){
  p1 = coef(fit1)[,,j]
  p2 = coef(fit2)[,,j]
...
```{r ref, fig.width = 3, fig.height = 3.7, out.width = "300px", fig.cap = "Scatterplot of normalised intensities after normalisation by reference ($x$-axis, `f8`) and joint normalisation ($y$-axis, `fall`). There is good agreement."}
...
```{r nkid-calib4, fig.width = 3, fig.height = 3.7, out.width = "300px", warning = FALSE, fig.cap = "Scatterplot for the badly biased `bkid` data: log-log scale."}
plot(exprs(bkid), main = "raw", pch = ".", log = "xy")
```
```{r nkid-calib5, fig.width = 3, fig.height = 3.7, out.width = "300px", warning = FALSE, fig.cap = "Scatterplot for the badly biased `bkid` data: after calibration and transformation with vsn."}
...
```{r vsnQ1, results = "hide", fig.width = 3, fig.height = 3.7, out.width = "300px", fig.cap = "Scatterplot between the red and green intensities of the array of the lymphoma dataset after quantile normalisation followed by VSN variance stabilisation without calibration. "} 
lym_q = normalizeQuantiles(exprs(lymphoma))
lym_qvsn = vsn2(lym_q, calib="none")
plot(exprs(lym_qvsn)[, 1:2], pch=".", main="lym_qvsn")
```
```{r vsnQ2, results = "hide", fig.width = 3, fig.height = 3.7, out.width = "300px", fig.cap = "Comparison of the values calculated in this section, for CH1 of the first array, to those of VSN variance stabilisation with affine calibration (`lym` was computed in Section \\@ref{sec:lymphoma})."} 
...
```{r shrink, results = "hide", fig.width = 5, fig.height = 4, out.width = "500px", echo = FALSE, fig.cap = "The shrinkage property of the generalised log-ratio.  Blue diamonds and error bars correspond to mean and standard deviation of the generalised log-ratio $h$, as obtained from VSN, and black dots and error bars to the standard log-ratio $q$ (both base 2)."}
...
```{r figgraph, results = "hide", fig.width = 3, fig.height = 3, out.width = "300px", echo = FALSE, fig.cap = "Graphs of the functions $y=\\log_2(x)$, $y=$glog$_2(x,c)=\\log_2(x+\\sqrt{x^2+c^2})-1$, and $y=\\log_2(x+x_{\\text{off}})$, where $c=50$ and $x_{\\text{off}}=50$."}
...
```{r lymbox, fig.width = 10, fig.height = 4.5, out.width = "750px", fig.cap = "Boxplot of A values of array CLL-13 stratified by within-sector row. The features in rows 22 and 23 are all very dim."}
