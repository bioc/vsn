...
knitr::opts_chunk$set(tidy = FALSE, cache = TRUE, autodep = TRUE,
    dev = "png", dpi = 600, message = FALSE, error = FALSE, warning = TRUE)
set.seed(0xdada)
```


<span style="color:red">This is an unfinished port of the package's "old" Sweave/PDF vignette into Rmarkdown/HTML. There are still bugs in here, like unevaluated LaTeX markup, unmapped cross-references, awkward figure dimensions and the bibliography. Please bear with me. In doubt, please consult the [existing PDF vignette](vsn.pdf).</span>
...
```{r nkid-sdmean1, fig.height = 3, fig.width = 3, out.width = "300px", fig.cap = "Standard deviation versus rank of the mean"}
meanSdPlot(xnorm, ranks = TRUE)
```
```{r nkid-sdmean2, fig.height = 3, fig.width = 3, out.width = "300px", fig.cap = "Standard deviation versus mean"}
...
``` 
```{r lym-sdmean, fig.height = 3, fig.width = 3, out.width = "300px", fig.cap = "Standard deviation versus rank of the mean for the lymphoma example data"}
meanSdPlot(lym)
```

We seethat the variance stabilisation worked.  As above, we can obtain the generalised 
log-ratios for each slide by subtracting the common reference intensities from those for the 8 samples:

```{r lym-M, fig.cap = "MA-plot for slide DLCL-0032 from the lymphoma example data. A false-colour representation of the data point density is used, in addition the 100 data points in the least dense regions are plotted as dots."}
iref = seq(1, 15, by=2)
ismp = seq(2, 16, by=2)
...
```{r affy, fig.width = 8, fig.height = 3, fig.cap = "Results of `vsnrma` and `rma` on the Dilution data. Array 1 was hybridised with `r as.character(pData(Dilution)$liver[1])` $\\mu$g RNA from liver, Array 3 with `r as.character(pData(Dilution)$liver[3])` $\\mu$g of the same RNA."}
par(mfrow = c(1, 3))
ax = c(2, 16)
...
```{r figgraph, results = "hide", echo = FALSE, fig.cap = "Graphs of the functions $y=\\log_2(x)$, $y=$glog$_2(x,c)=\\log_2(x+\\sqrt{x^2+c^2})-1$, and $y=\\log_2(x+x_{\\mbox{\\scriptsize off}})$, where $c=\\Sexpr{cc}$ and $x_{\\mbox{\\scriptsize off}}=`r xoff`$."}
...
`loess` normalization of reference~\cite{YeeNorm}
